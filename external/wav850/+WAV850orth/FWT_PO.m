...
%    IWT_PO, MakeONFilter
%
  [n,J] = WAV850orth.dyadlength(x) ;
  wcoef = zeros(1,n) ;
  beta = WAV850util.ShapeAsRow(x);  %take samples at finest scale as beta-coeffts
  for j=J-1:-1:L
       alfa = WAV850orth.DownDyadHi(beta,qmf);
       wcoef(WAV850orth.dyad(j)) = alfa;
       beta = WAV850orth.DownDyadLo(beta,qmf) ;  
  end
  wcoef(1:(2^L)) = beta;
  wcoef = WAV850util.ShapeLike(wcoef,x);
